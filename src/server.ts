...
IMPORTANT: Always use ToolSearch to fetch full parameter schemas before calling any cc-mess tool. Tool parameters:
- send: to (string), type (chat|task|result|review|insight), subject (string), body (string), priority? (normal|high|low)
- broadcast: type (insight|announcement), subject (string), body (string), filter? (object)
- reply: message_id (string), body (string)
- spawn: cwd (string), task (string), capabilities? (string[]), hooks? (strict|permissive)
- kill: target (string), reason (string)
- list_instances: (no params)
- update_self: task? (string), capabilities? (string[])
- mesh_control: action (pause|resume|shutdown_all)
- send_as_human: to (string), body (string)`
    : undefined;
