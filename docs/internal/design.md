...
- Launches via `child_process.spawn()` with an argv array — never shell interpolation:
  ```ts
  spawn("claude", ["--dangerously-skip-permissions", task], { shell: false, cwd })
  ```
  Uses a positional prompt argument (interactive mode) so the instance stays alive after responding. The `cwd` option on the spawn call sets the working directory (the Claude CLI has no `--cwd` flag).
...
- When `CC_MESS_TMUX_SESSION` is set (dashboard mode), spawns inside a tmux pane instead of headless, giving the instance a real TTY
- The new instance joins the mesh automatically via `.mcp.json` in the cwd
