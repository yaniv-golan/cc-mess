...
describe("transport", () => {
  ...
  describe("formatChannelNotification", () => {
    it("formats a regular message", () => {
      ...
      expect(notif.content).toContain("Check the auth module");
      expect(notif.meta.source).toBe("mesh");
      expect(notif.meta.from).toBe("apollo-3f2a");
      expect(notif.meta.type).toBe("task");
    });
    ...
    it("formats a control message", () => {
      ...
      const notif = formatChannelNotification(msg);
      expect(notif.meta.type).toBe("control");
      expect(notif.meta.action).toBe("shutdown");
      expect(notif.content).toContain("Done");
    });
  });
